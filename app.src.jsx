...
const EXP = {
  W: 1080, PAD: 92,
  ink: '#f2f0ea', dim: '#8a8a8a', line: '#2a2a2a',
  card: '#f5f3ee', cardInk: '#111', cardRed: '#c4322b',
  bevel: '#d9d5c9', shadow: '#23231f',
};
...
function expDrawCard(ctx, x, y, rank, suit){
  ...
  const map = PIXEL_SPRITES[suit], px = 4;
  const sw = map[0].length * px, sh = map.length * px;
  const sx = Math.round(x + (Cw - sw)/2), sy = Math.round(y + (Ch - sh)/2 + 15);
  map.forEach((row, ry) => {
    for(let rx = 0; rx < row.length; rx++)
      if(row[rx] === '#') ctx.fillRect(sx + rx*px, sy + ry*px, px, px);
  });
}

// A sparse band of the app's pixel sparkles across the top of the log.
function expDrawStars(ctx, W){
  const gold = '#e8c98a', blue = '#a8c2eb';
  const plot = (x, y, cells, cellPx, color) => {
    ctx.fillStyle = color;
    cells.forEach(([cx, cy, w, h]) =>
      ctx.fillRect(Math.round(x + cx*cellPx), Math.round(y + cy*cellPx), w*cellPx, h*cellPx));
  };
...
function paintJournalExport(ctx, p, sketchImg, measureOnly){
  ...
  if(draw){ ctx.fillStyle = '#000'; ctx.fillRect(0, 0, W, ctx.canvas.height); }
  ...
  if(draw){ ctx.fillStyle = EXP.ink; ctx.fillText(p.name.toUpperCase(), W/2, y + 46); }
  y += 46 + 40;
  ...
  if(draw){ ctx.fillStyle = EXP.dim; ctx.fillText(sub, W/2, y + 30); }
  y += 30 + 64;
  ctx.textAlign = 'left';
  if(draw){ ctx.fillStyle = EXP.line; ctx.fillRect(PAD, y, CW, 3); }
  y += 3;
  p.entries.forEach((e, i) => {
    if(i > 0){ if(draw){ ctx.fillStyle = EXP.line; ctx.fillRect(PAD, y, CW, 3); } y += 3; }
    y += 56;
    ...
    if(promptText){
      ...
      if(draw){ ctx.fillStyle = EXP.dim; lines.forEach((ln, k) => ctx.fillText(ln, PAD, y + 30 + k*46)); }
      y += lines.length * 46 + 26;
    }
    if(e.body){
      ctx.font = '32px "Space Mono", monospace';
      const lines = expWrap(ctx, e.body, CW);
      if(draw){ ctx.fillStyle = EXP.ink; lines.forEach((ln, k) => ctx.fillText(ln, PAD, y + 32 + k*56)); }
      y += lines.length * 56;
    }
    y += 48;
  });
  if(sketchImg){
    ...
    if(draw){
      ctx.fillStyle = EXP.shadow; ctx.fillRect(PAD + 13, y + 13, CW, sh);
      ctx.fillStyle = EXP.card; ctx.fillRect(PAD, y, CW, sh);
      ctx.imageSmoothingEnabled = false;
...
function App(){
  ...
  async function shareJournal(p){
    ...
    try{
      ...
      const slug = (p.name || '').toLowerCase().replace(/[^a-z0-9]+/g, '-')
        .replace(/^-+|-+$/g, '') || 'journal';
      const file = new File([blob], slug + '-log.png', { type: 'image/png' });
    ...
    }catch(e){ /* export failed; leave the journal untouched */ }
  ...
  function onRestoreFile(ev){
    ...
    reader.onload = () => {
      ...
      try{
        ...
        if(data && data.format === ARCHIVE_FORMAT && Array.isArray(data.planets)) planets = data.planets;
        else if(Array.isArray(data)) planets = data;   // a raw archive array also restores
      }catch(e){}
      if(!planets){ setToast("This doesn't look like a ship's log."); return; }
      ...
      <div className="screen with-chrome fade-in">
        ...
        {archive.length === 0 ? (
          <div className="center">
            ...
            <button className="btn small" style={{marginTop:'22px'}} onClick={requestRestore}>Restore archive</button>
          </div>
        ) : (
          <>
  ...
  function renderFullJournal(){
    ...
    return (
      <div className="screen fade-in">
        <button className="close-x" aria-label="Close" onClick={() => { setToast(null); setScreen('archive'); }}>✕</button>
        ...
        <div className="fj-actions-row">
          <button className="btn danger" onClick={() => setOverlay('burn')}>Burn journal</button>
          <button className="btn primary" disabled={sharing} style={{opacity: sharing ? .4 : 1}}
            onClick={() => shareJournal(p)}>Share</button>
        </div>
      </div>
    );
  }
...
function SketchScreen({ onAdd, onSkip, onBack, onHome, onHelp }){
  ...
  const last = useRef({ x:0, y:0 });   // last CELL the pointer touched
